...
class Player(GameEntity):
    ...
    def __repr__(self):
        return f"['{self.__class__.__name__}', {self.position}, {self.rotation}, {self.team}, {self.score}, {self.eliminations}, {self.hp}, {self.next_attack_timestamps}, {self.can_move}, {self.can_attack}]"
    ...
    def create(parsed_object) -> Player:
        try:
            ...
            entity.can_move = bool(parsed_object[7])
            entity.can_attack = bool(parsed_object[8])
            return entity
        except:
            return None
